...
class TestProfileMappingResource:
    ...
    async def test_list_profile_mappings(self, fs):
        ...
        if err and "404" in str(err):
            ...
            pytest.skip("Profile mappings not available in this environment (404 error)")
    ...
    async def test_update_profile_mapping(self, fs):
        ...
        original_properties = dict(current_mapping.properties) if current_mapping.properties else {}

        # Check the source and target to determine valid field mappings
        source_type = getattr(current_mapping.source, "type", "user") if current_mapping.source else "user"
        target_type = getattr(current_mapping.target, "type", "user") if current_mapping.target else "user"
        ...
        try:
            ...
            if err:
                ...
                if "Invalid target field" in error_msg:
                    ...
                    if field_to_update != "email":
                        field_to_update = "email"
                        test_expression = "user.email" if source_type == "user" else "appuser.email"
                        updated_properties[field_to_update] = models.ProfileMappingProperty(expression=test_expression)
                        profile_mapping_request = models.ProfileMappingRequest(properties=updated_properties)
                        updated_mapping, _, err = await client.update_profile_mapping(mapping_id, profile_mapping_request)

                    if err:
                        pytest.skip(f"Cannot find valid field to map for this profile mapping type")
                    ...
                    if "appuser.email" in error_msg:
                        ...
                        test_expression = "appuser.userName" if source_type == "appuser" else "user.login"
                        updated_properties[field_to_update] = models.ProfileMappingProperty(expression=test_expression)
                        profile_mapping_request = models.ProfileMappingRequest(properties=updated_properties)
                        updated_mapping, _, err = await client.update_profile_mapping(mapping_id, profile_mapping_request)
                ...
                elif "Unable to resolve user" in error_msg or "Unable to resolve appuser" in error_msg:
                    pytest.skip(f"Expression '{test_expression}' not valid for this profile mapping type")
                elif "Attributes from the same profile cannot be mapped" in error_msg:
                    pytest.skip("Cannot map attributes from same profile in this mapping")
                elif "Push status is allowed for profile mastering enabled apps only" in error_msg:
            ...
            assert updated_mapping.properties[field_to_update].expression == test_expression

            # Only check push_status if it was supported
            if supports_push_status:
                expected_push_status = models.ProfileMappingPropertyPushStatus.DONT_PUSH
                assert updated_mapping.properties[field_to_update].push_status == expected_push_status
            ...
            try:
                ...
                _, _, err = await client.update_profile_mapping(mapping_id, restore_request)
                if err:
                    print(f"Warning: Could not restore original mapping properties: {err}")
    ...
    async def test_remove_property_from_profile_mapping(self, fs):
        ...
        try:
            ...
            if err:
                error_msg = str(err)
                if "Invalid target field" in error_msg or "Invalid property" in error_msg:
                    # Try with a different expression
                    fallback_expressions = ["user.email", "user.login"] if source_type == "user" else ["appuser.email", "appuser.userName"]
                    ...
                    for fallback_expr in fallback_expressions:
                        if supports_push_status:
                            ...
                            temp_property = models.ProfileMappingProperty(expression=fallback_expr)

                        properties_with_temp["tempProperty"] = temp_property
                        add_request = models.ProfileMappingRequest(properties=properties_with_temp)
                        updated_mapping, _, err = await client.update_profile_mapping(mapping_id, add_request)

                        if err is None:
                            break

                    if err:
                        pytest.skip(f"Cannot find valid expression for this profile mapping type: {error_msg}")

                elif "Push status is allowed for profile mastering enabled apps only" in error_msg:
                    # Remove pushStatus and try again
                    temp_property = models.ProfileMappingProperty(expression=test_expression)
                    properties_with_temp["tempProperty"] = temp_property
                    add_request = models.ProfileMappingRequest(properties=properties_with_temp)
                    updated_mapping, _, err = await client.update_profile_mapping(mapping_id, add_request)

                    if err:
                        pytest.skip("Cannot add temporary property even without pushStatus")

                elif any(phrase in error_msg for phrase in ["Unable to resolve", "Attributes from the same profile cannot be mapped"]):
                    pytest.skip(f"Expression not valid for this profile mapping type: {error_msg}")

                else:
                    pytest.skip(f"Unexpected error when adding temporary property: {error_msg}")
            ...
            properties_without_temp = {k: v for k, v in properties_with_temp.items() if k != "tempProperty"}
            ...
            if final_mapping.properties:
                assert "tempProperty" not in final_mapping.properties, "Temporary property was not removed"
            ...
            try:
                ...
                _, _, restore_err = await client.update_profile_mapping(mapping_id, restore_request)
                if restore_err:
                    print(f"Warning: Could not restore original mapping properties: {restore_err}")
            except Exception as exc:
                print(f"Warning: Could not restore original mapping properties: {exc}")
    ...
    async def test_filter_profile_mappings_by_source(self, fs):
        ...
        source_id = getattr(all_mappings[0].source, "id", None) if all_mappings[0].source else None

        if source_id:
            # Filter by source ID
            filtered_mappings, _, err = await client.list_profile_mappings(source_id=source_id)
            assert err is None
    ...
    async def test_filter_profile_mappings_by_target(self, fs):
        ...
        target_id = getattr(all_mappings[0].target, "id", None) if all_mappings[0].target else None

        if target_id:
            # Filter by target ID
            filtered_mappings, _, err = await client.list_profile_mappings(target_id=target_id)
            assert err is None
            assert isinstance(filtered_mappings, list)
    ...
    async def test_profile_mapping_with_http_info(self, fs):
        ...
        try:
            ...
            profile_mappings, response, err = await client.list_profile_mappings_with_http_info()
            ...
            if len(profile_mappings) > 0:
                ...
                profile_mapping, response, err = await client.get_profile_mapping_with_http_info(mapping_id)
                assert err is None
                assert isinstance(profile_mapping, models.ProfileMapping)
                assert response is not None
                assert hasattr(response, 'status_code') and response.status_code == 200
    ...
    async def test_update_profile_mapping_with_push_status_variations(self, fs):
        ...
        if not supports_push_status:
            ...
            _, _, test_err = await client.update_profile_mapping(mapping_id, test_request)

            if test_err and "Push status is allowed for profile mastering enabled apps only" in str(test_err):
                # This mapping doesn't support push status, skip the test
                pytest.skip("Profile mapping does not support push status - profile mastering not enabled")
            elif test_err is None:
                # Push status is supported, clean up the test property
                supports_push_status = True
                restore_request = models.ProfileMappingRequest(properties=original_properties)
                await client.update_profile_mapping(mapping_id, restore_request)

        if not supports_push_status:
            pytest.skip("Unable to determine push status support for this profile mapping")
        ...
        try:
            ...
            for push_status in push_statuses:
                ...
                if err:
                    error_msg = str(err)
                    if "Push status is allowed for profile mastering enabled apps only" in error_msg:
                        pytest.skip("Push status not supported for this profile mapping")
                    elif "Invalid target field" in error_msg or "Invalid property" in error_msg:
                        # Try with a different expression
                        test_property = models.ProfileMappingProperty(
                        ...
                        if err:
                            pytest.skip(f"Cannot test push status with available expressions: {error_msg}")
                    else:
                        pytest.skip(f"Unexpected error testing push status: {error_msg}")

                # Verify the update was successful
                assert err is None, f"Failed to update profile mapping with push status {push_status}: {err}"
                assert isinstance(updated_mapping, models.ProfileMapping)

                if updated_mapping.properties and test_field_name in updated_mapping.properties:
                    assert updated_mapping.properties[test_field_name].push_status == push_status
