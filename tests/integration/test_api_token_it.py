...
class TestApiTokenResource:
    ...
    static_token_id = "00T1f3w86v9h8o1VZ1d7"  # Use a static token ID for consistent testing
    ...
    async def test_list_api_tokens_basic(self, fs):
        ...
        if tokens:
            TestApiTokenResource.discovered_token_ids = [token.id for token in tokens[:3]]  # Store up to 3 for testing
    ...
    async def test_list_api_tokens_with_http_info(self, fs):
        ...
        tokens, http_response, err = await client.list_api_tokens_with_http_info(limit=3)
    ...
    async def test_revoke_api_token_method_exists(self, fs):
        ...
        try:
            result = await client.revoke_api_token(invalid_token_id)
            ...
            assert "404" in str(err) or "not found" in str(err).lower() or "invalid" in str(err).lower() or "method" in str(err).lower()
    ...
    async def test_revoke_api_token_with_http_info_method_exists(self, fs):
        ...
        try:
            result = await client.revoke_api_token_with_http_info(invalid_token_id)
            # If we get a successful result with invalid ID, just verify method exists
    ...
    async def test_revoke_api_token_without_preload_content_method_exists(self, fs):
        ...
        try:
            result = await client.revoke_api_token_without_preload_content(invalid_token_id)
            # If we get a successful result with invalid ID, just verify method exists
    ...
    @pytest.mark.vcr()
    @pytest.mark.asyncio
    async def test_revoke_current_api_token_without_preload_content_method_exists(self, fs):
        ...
        assert callable(getattr(client, 'revoke_current_api_token_without_preload_content'))
    ...
    async def test_error_handling_scenarios(self, fs):
        ...
        for invalid_id in invalid_ids:
            try:
                ...
                assert any(code in str(e) for code in ["400", "404", "invalid", "bad", "validation"])
    ...
    async def test_http_info_response_structure(self, fs):
        ...
        tokens, http_response, err = await client.list_api_tokens_with_http_info(limit=2)
        assert err is None
    ...
    async def test_without_preload_content_variants(self, fs):
        ...
        tokens_no_preload, _, err = await client.list_api_tokens_without_preload_content(limit=2)
        assert err is None
        assert isinstance(tokens_no_preload, list)
        
        # Test get without preload content using static token ID
        token_id = TestApiTokenResource.static_token_id
        token_no_preload, _, err = await client.get_api_token_without_preload_content(token_id)
        assert err is None
