...
class UserGroupsTest(unittest.TestCase):
    ...
    def test_requests_groups(self):
        client = build_client("/api/v1/groups - requests groups")
        client.get_groups()
    ...
    def test_updates_a_group(self):
        ...
        groups = client.get_groups()
        group_ids = [group.id for group in groups if group.profile.name == "TestGroup"]
        updated_group = {
            "profile": {
                "name": "TestGroup",
    ...
    def test_delete_group(self):
        ...
        groups = client.get_groups()
        group_ids = [group.id for group in groups if group.profile.name == "DeleteGroup"]
    ...
    def test_add_user_to_group(self):
        client = build_client("/api/v1/groups/:id - add user to group")
        user_client = build_users_client("/api/v1/groups/:id - add user to group")
        group = [group for group in client.get_groups() if group.profile.name == "TestGroup"]
        user = user_client.get_user('frutis.mcjanky@example.com')
