...
class UserGroupsClient(ApiClient):
	...
	def get_users_by_group (self, gid, limit=None, query=None, filter_string=None):
		
		response = ApiClient.get_path(self, '/{0}'.format(gid) + '/users')
		
		response = Utils.deserialize(response.text, User)
