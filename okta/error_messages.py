...
ERROR_MESSAGE_ORG_URL_MISSING = (
    f"Your Okta URL is missing. You can copy your domain from the Okta Developer Console. Follow these instructions to find it: {FINDING_OKTA_DOMAIN}"
)
...
ERROR_MESSAGE_ORG_URL_YOUROKTADOMAIN = (
    f"Replace {{yourOktaDomain}} with your Okta domain. You can copy your domain from the Okta Developer Console. Follow these instructions to find it: {FINDING_OKTA_DOMAIN}"
)
...
ERROR_MESSAGE_API_TOKEN_MISSING = (
    f"Your Okta API token is missing. You can generate one in the Okta Developer Console. Follow these instructions: {GET_OKTA_API_TOKEN}"
)

ERROR_MESSAGE_API_TOKEN_DEFAULT = (
    f"Replace {{apiToken}} with your Okta API token. You can generate one in the Okta Developer Console. Follow these instructions: {GET_OKTA_API_TOKEN}"
)
...
ERROR_MESSAGE_CLIENT_ID_DEFAULT = (
    f"Replace {{clientId}} with the client ID of your Application. You can copy it from the Okta Developer Console in the details for the Application you created. Follow these instructions to find it: {FINDING_OKTA_APP_CRED}"
)

ERROR_MESSAGE_SCOPES_PK_MISSING = (
    "When using authorization mode &#39;PrivateKey&#39;, you must supply &#39;okta.client.scopes&#39; and &#39;okta.client.privateKey&#39;"
)
