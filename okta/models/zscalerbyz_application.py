...
class ZscalerbyzApplication(BaseModel):
    """
    Schema for the Zscaler 2.0 app (key name: `zscalerbyz`)  To create a Zscaler 2.0 app, use the [Create an Application](
    /openapi/okta-management/management/tag/Application/#tag/Application/operation/createApplication) request with the
    following parameters in the request body. > **Note:** The Zscaler 2.0 app only supports `BROWSER_PLUGIN` and
    `SAML_2_0` sign-on modes.
    """  # noqa: E501
    ...
    def name_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['zscalerbyz']):
            raise ValueError("must be one of enum values ('zscalerbyz')")
        return value
    ...
    def sign_on_mode_validate_enum(cls, value):
        ...
        if value not in set(['BROWSER_PLUGIN', 'SAML_2_0']):
            raise ValueError("must be one of enum values ('BROWSER_PLUGIN', 'SAML_2_0')")
        return value
        ...
        _obj = cls.model_validate(
            {
                ...
                "settings": ZscalerbyzApplicationSettings.from_dict(obj["settings"]) if obj.get(
                    "settings"
                ) is not None else None
            }
        )
