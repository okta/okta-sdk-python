...
class AuthenticatorMethodOtp(AuthenticatorMethodWithVerifiableProperties):
    ...
    Field(
        ...
        description="The number of acceptable adjacent intervals, also known as the clock drift interval. This setting "
                    "allows you to build in tolerance for any time difference between the token and the server. For "
                    "example, with a `timeIntervalInSeconds` of 60 seconds and an `acceptableAdjacentIntervals` value of "
                    "5, Okta accepts passcodes within 300 seconds (60 * 5) before or after the end user enters their code.",
