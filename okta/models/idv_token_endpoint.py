...
class IDVTokenEndpoint(BaseModel):
    """
    ...
    url: Optional[StrictStr] = Field(
        default=None, description="URL of the `token` endpoint of the IDV vendor"
    )
    ...
    def binding_validate_enum(cls, value):
        ...
        if value not in set(["HTTP-POST"]):
            raise ValueError("must be one of enum values ('HTTP-POST')")
        return value
