...
class ProvisioningConditions(BaseModel):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.deprovisioned:
            if not isinstance(self.deprovisioned, dict):
        ...
        if self.suspended:
            if not isinstance(self.suspended, dict):
                _dict["suspended"] = self.suspended.to_dict()
            else:
                _dict["suspended"] = self.suspended
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "deprovisioned": (
                    ProvisioningDeprovisionedCondition.from_dict(obj["deprovisioned"])
                    if obj.get("deprovisioned") is not None
                    else None
                ),
                "suspended": (
                    ProvisioningSuspendedCondition.from_dict(obj["suspended"])
                    if obj.get("suspended") is not None
                    else None
                ),
            }
        )
