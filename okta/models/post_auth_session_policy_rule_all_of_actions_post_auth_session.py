...
class PostAuthSessionPolicyRuleAllOfActionsPostAuthSession(BaseModel):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in failure_actions (list)
        _items = []
        if self.failure_actions:
            for _item in self.failure_actions:
                if _item:
                    _items.append(_item.to_dict())
            _dict["failureActions"] = _items
        return _dict
        ...
        _obj = cls.model_validate(
            {
                "failureActions": (
                    [
                        PostAuthSessionFailureActionsObject.from_dict(_item)
                        for _item in obj["failureActions"]
                    ]
                    if obj.get("failureActions") is not None
                    else None
                )
            }
        )
