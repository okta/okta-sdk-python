...
class OAuthEndpoints(BaseModel):
    """
    The `OAUTH2` and `OIDC` protocols support the `authorization` and `token` endpoints. Also, the `OIDC` protocol supports
    the `userInfo` and `jwks` endpoints.  The IdP Authorization Server (AS) endpoints are currently defined as part of the [
    IdP provider]((https://developer.okta.com/docs/api/openapi/okta-management/management/tag/IdentityProvider/#tag
    /IdentityProvider/operation/createIdentityProvider!path=type&t=request)) and are read-only.
    """  # noqa: E501
    ...
    token: Optional[OAuthTokenEndpoint] = None
    user_info: Optional[OidcUserInfoEndpoint] = Field(default=None, alias="userInfo")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "authorization": (
                    OAuthAuthorizationEndpoint.from_dict(obj["authorization"])
                    if obj.get("authorization") is not None
                    else None
                ),
                "jwks": (
                    OidcJwksEndpoint.from_dict(obj["jwks"])
                    if obj.get("jwks") is not None
                    else None
                ),
                "slo": (
                    OidcSloEndpoint.from_dict(obj["slo"])
                    if obj.get("slo") is not None
                    else None
                ),
                "token": (
                    OAuthTokenEndpoint.from_dict(obj["token"])
                    if obj.get("token") is not None
                    else None
                ),
                "userInfo": (
                    OidcUserInfoEndpoint.from_dict(obj["userInfo"])
