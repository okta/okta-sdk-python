...
class OrgContactTypeObj(BaseModel):
    ...
    # data type: OrgBillingContactType
    anyof_schema_1_validator: Optional[OrgBillingContactType] = None
    # data type: OrgTechnicalContactType
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = OrgContactTypeObj.model_construct()
        error_messages = []
        # validate data type: OrgBillingContactType
        if not isinstance(v, OrgBillingContactType):
            error_messages.append(f"Error! Input type `{type(v)}` is not `OrgBillingContactType`")
        else:
            return v

        # validate data type: OrgTechnicalContactType
        if not isinstance(v, OrgTechnicalContactType):
            error_messages.append(f"Error! Input type `{type(v)}` is not `OrgTechnicalContactType`")
        else:
            return v
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], OrgBillingContactType, OrgTechnicalContactType]]:
