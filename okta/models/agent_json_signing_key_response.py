...
class AgentJsonSigningKeyResponse(BaseModel):
    ...
    """
    # data type: AgentJsonWebKeyRsaResponse
    oneof_schema_1_validator: Optional[AgentJsonWebKeyRsaResponse] = None
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = AgentJsonSigningKeyResponse.model_construct()
        error_messages = []
        match = 0
        # validate data type: AgentJsonWebKeyRsaResponse
        if not isinstance(v, AgentJsonWebKeyRsaResponse):
            error_messages.append(f"Error! Input type `{type(v)}` is not `AgentJsonWebKeyRsaResponse`")
        else:
            match += 1
        # validate data type: AgentJsonWebKeyECResponse
        if not isinstance(v, AgentJsonWebKeyECResponse):
            error_messages.append(f"Error! Input type `{type(v)}` is not `AgentJsonWebKeyECResponse`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # check if data type is `AgentJsonWebKeyECResponse`
        if _data_type == "EC":
            instance.actual_instance = AgentJsonWebKeyECResponse.from_json(json_str)
            return instance

        # check if data type is `AgentJsonWebKeyRsaResponse`
        if _data_type == "RSA":
            instance.actual_instance = AgentJsonWebKeyRsaResponse.from_json(json_str)
            return instance

        # check if data type is `AgentJsonWebKeyECResponse`
        if _data_type == "AgentJsonWebKeyECResponse":
            instance.actual_instance = AgentJsonWebKeyECResponse.from_json(json_str)
            return instance

        # check if data type is `AgentJsonWebKeyRsaResponse`
        if _data_type == "AgentJsonWebKeyRsaResponse":
            instance.actual_instance = AgentJsonWebKeyRsaResponse.from_json(json_str)
            return instance

        # deserialize data into AgentJsonWebKeyRsaResponse
        try:
            instance.actual_instance = AgentJsonWebKeyRsaResponse.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into AgentJsonWebKeyECResponse
        try:
            instance.actual_instance = AgentJsonWebKeyECResponse.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], AgentJsonWebKeyECResponse, AgentJsonWebKeyRsaResponse]]:
