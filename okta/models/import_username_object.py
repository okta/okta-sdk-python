...
class ImportUsernameObject(BaseModel):
    ...
    def username_format_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(["EMAIL", "CUSTOM"]):
            raise ValueError("must be one of enum values ('EMAIL', 'CUSTOM')")
        return value
        ...
        _obj = cls.model_validate(
            {
                ...
                "usernameFormat": (
                    obj.get("usernameFormat")
                    if obj.get("usernameFormat") is not None
                    else "EMAIL"
                ),
            }
        )
