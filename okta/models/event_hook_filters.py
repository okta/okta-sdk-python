...
class EventHookFilters(BaseModel):
    ...
    EA)](/openapi/okta-management/guides/release-lifecycle/#early-access-ea) to enable. If you want to disable this feature,
    it's recommended to first remove all event filters.
    """  # noqa: E501
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * OpenAPI `readOnly` fields are excluded.
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in event_filter_map (list)
        _items = []
        if self.event_filter_map:
            for _item in self.event_filter_map:
                if _item:
                    _items.append(_item.to_dict())
            _dict["eventFilterMap"] = _items
        return _dict
        ...
        _obj = cls.model_validate(
            {
                "eventFilterMap": (
                    [
                        EventHookFilterMapObject.from_dict(_item)
                        for _item in obj["eventFilterMap"]
                    ]
                    if obj.get("eventFilterMap") is not None
                    else None
                ),
                "type": obj.get("type"),
            }
