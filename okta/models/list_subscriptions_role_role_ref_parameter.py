...
class ListSubscriptionsRoleRoleRefParameter(BaseModel):
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if not isinstance(v, RoleType):
            error_messages.append(f"Error! Input type `{type(v)}` is not `RoleType`")
        else:
            match += 1
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when setting `actual_instance` in ListSubscriptionsRoleRoleRefParameter with oneOf "
                "schemas: RoleType, str. Details: " + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when setting `actual_instance` in ListSubscriptionsRoleRoleRefParameter with oneOf schemas: "
                "RoleType, str. Details: " + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into RoleType
        try:
            instance.actual_instance = RoleType.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], RoleType, str]]:
