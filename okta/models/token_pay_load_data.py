...
class TokenPayLoadData(BaseModel):
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "context": (
                    TokenPayLoadDataContext.from_dict(obj["context"])
                    if obj.get("context") is not None
                    else None
                ),
                "identity": (
                    TokenPayLoadDataIdentity.from_dict(obj["identity"])
                    if obj.get("identity") is not None
                    else None
                ),
                "access": (
                    TokenPayLoadDataAccess.from_dict(obj["access"])
                    if obj.get("access") is not None
                    else None
                ),
                "refresh_token": (
                    RefreshToken.from_dict(obj["refresh_token"])
