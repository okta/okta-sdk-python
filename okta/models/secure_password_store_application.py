...
class SecurePasswordStoreApplication(Application):
    ...
    name: StrictStr = Field(description="`template_sps` is the key name for a SWA app instance that uses HTTP POST and doesn't require a browser plugin")
    ...
    def name_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['template_sps']):
            raise ValueError("must be one of enum values ('template_sps')")
        return value
