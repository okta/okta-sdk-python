...
class OpenIdConnectApplication(Application):
    ...
    def name_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['oidc_client']):
            raise ValueError("must be one of enum values ('oidc_client')")
        return value
        ...
        _obj = cls.model_validate(
            {
                ...
                "credentials": OAuthApplicationCredentials.from_dict(obj["credentials"]) if obj.get(
                    "credentials"
                ) is not None else None,
                "name": obj.get("name"),
                "settings": OpenIdConnectApplicationSettings.from_dict(obj["settings"]) if obj.get(
                    "settings"
                ) is not None else None
            }
        )
