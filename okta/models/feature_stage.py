...
class FeatureStage(BaseModel):
    ...
    the value is `BETA`, the state is `OPEN` or `CLOSED` depending on whether the `BETA` feature is manageable.  >
    **Note:** If a feature's stage is `OPEN BETA`, you can update it only in Preview cells. If a feature's stage is
    `CLOSED BETA`, you can disable it only in Preview cells.
    """  # noqa: E501
    state: Optional[FeatureStageState] = None
    value: Optional[FeatureStageValue] = None
    __properties: ClassVar[List[str]] = ["state", "value"]
