...
class GoogleApplication(BaseModel):
    """
    Schema for the Google Workspace app (key name: `google`)  To create a Google Workspace app, use the [Create an
    Application](https://developer.okta.com/docs/api/openapi/okta-management/management/tag/Application/#tag/Application
    /operation/createApplication) request with the following parameters in the request body. > **Note:** The Google
    Workspace app only supports `BROWSER_PLUGIN` and `SAML_2_0` sign-on modes.
    """  # noqa: E501
    ...
    def sign_on_mode_validate_enum(cls, value):
        ...
        if value not in set(["BROWSER_PLUGIN", "SAML_2_0"]):
            raise ValueError(
                "must be one of enum values ('BROWSER_PLUGIN', 'SAML_2_0')"
            )
        return value
        ...
        _obj = cls.model_validate(
            {
                ...
                "name": obj.get("name") if obj.get("name") is not None else "google",
                ...
                "settings": (
                    GoogleApplicationSettings.from_dict(obj["settings"])
