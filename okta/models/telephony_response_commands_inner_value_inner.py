...
class TelephonyResponseCommandsInnerValueInner(BaseModel):
    ...
    status: Optional[StrictStr] = Field(
        default=None, description="Status of telephony callout"
    )
    provider: Optional[StrictStr] = Field(
        default=None, description="Telephony provider for sms/voice"
    )
    transaction_id: Optional[StrictStr] = Field(
        default=None, description="Transaction ID for sms/voice", alias="transactionId"
    )
    ...
    def status_validate_enum(cls, value):
        ...
        if value not in set(["SUCCESSFUL", "PENDING", "FAILED"]):
            raise ValueError(
                "must be one of enum values ('SUCCESSFUL', 'PENDING', 'FAILED')"
            )
        return value
