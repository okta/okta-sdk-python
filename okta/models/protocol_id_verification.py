...
class ProtocolIdVerification(BaseModel):
    ...
    type: Optional[StrictStr] = Field(
        default=None, description="ID verification protocol"
    )
    ...
    def type_validate_enum(cls, value):
        ...
        if value not in set(["ID_PROOFING"]):
            raise ValueError("must be one of enum values ('ID_PROOFING')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "credentials": (
                    IDVCredentials.from_dict(obj["credentials"])
                    if obj.get("credentials") is not None
                    else None
                ),
                "endpoints": (
                    IDVEndpoints.from_dict(obj["endpoints"])
