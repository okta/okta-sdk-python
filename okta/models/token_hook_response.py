...
class TokenHookResponse(BaseModel):
    ...
    commands: Optional[List[TokenHookResponseCommandsInner]] = Field(
        default=None,
        description="You can use the `commands` object to provide commands to Okta. It's where you can tell Okta to add more "
        "claims to the token. The `commands` object is an array, allowing you to send multiple commands. In each "
        "array element, there needs to be a `type` property and `value` property. The `type` property is where "
        "you specify which of the supported commands you want to execute, and `value` is where you supply an "
        "operand for that command. In the case of the token hook type, the `value` property is itself a nested "
        "object in which you specify a particular operation, a path to act on, and a value.",
    )
        ...
        _obj = cls.model_validate(
            {
                "commands": (
                    [
                        TokenHookResponseCommandsInner.from_dict(_item)
                ...
                "error": (
                    TokenHookResponseError.from_dict(obj["error"])
