...
class TrendMicroApexOneServiceApplicationSettings(BaseModel):
    ...
    def em_opt_in_status_validate_enum(cls, value):
        ...
        if value not in set(['DISABLED', 'DISABLING', 'ENABLED', 'ENABLING', 'NONE']):
            raise ValueError("must be one of enum values ('DISABLED', 'DISABLING', 'ENABLED', 'ENABLING', 'NONE')")
        return value
        ...
        _obj = cls.model_validate(
            {
                ...
                "app": TrendMicroApexOneServiceApplicationSettingsApplication.from_dict(obj["app"]) if obj.get(
                    "app"
                ) is not None else None,
                "signOn": OINSaml20ApplicationSettingsSignOn.from_dict(obj["signOn"]) if obj.get(
                    "signOn"
                ) is not None else None
            }
        )
