...
class SamlAttributeStatement(BaseModel):
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = SamlAttributeStatement.model_construct()
        error_messages = []
        # validate data type: SamlAttributeStatementExpression
        if not isinstance(v, SamlAttributeStatementExpression):
            error_messages.append(f"Error! Input type `{type(v)}` is not `SamlAttributeStatementExpression`")
        else:
            return v

        # validate data type: SamlAttributeStatementGroup
        if not isinstance(v, SamlAttributeStatementGroup):
            error_messages.append(f"Error! Input type `{type(v)}` is not `SamlAttributeStatementGroup`")
        else:
            return v
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], SamlAttributeStatementExpression, SamlAttributeStatementGroup]]:
