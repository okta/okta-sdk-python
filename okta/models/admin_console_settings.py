...
class AdminConsoleSettings(BaseModel):
    ...
    session_idle_timeout_minutes: Optional[
        Annotated[int, Field(le=720, strict=True, ge=5)]
    ] = Field(
        default=15,
        description="The maximum idle time before the Okta Admin Console session expires. Must be no more than 12 hours.",
        alias="sessionIdleTimeoutMinutes",
    )
    session_max_lifetime_minutes: Optional[
        Annotated[int, Field(le=10080, strict=True, ge=5)]
    ] = Field(
        default=720,
        description="The absolute maximum session lifetime of the Okta Admin Console. Must be no more than 7 days.",
        alias="sessionMaxLifetimeMinutes",
    )
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "sessionIdleTimeoutMinutes": (
                    obj.get("sessionIdleTimeoutMinutes")
                    if obj.get("sessionIdleTimeoutMinutes") is not None
                    else 15
                ),
                "sessionMaxLifetimeMinutes": (
                    obj.get("sessionMaxLifetimeMinutes")
                    if obj.get("sessionMaxLifetimeMinutes") is not None
                    else 720
                ),
            }
        )
