...
class FulfillmentRequest(BaseModel):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in fulfillment_data (list)
        _items = []
        if self.fulfillment_data:
            for _item in self.fulfillment_data:
                if _item:
                    _items.append(_item.to_dict())
            _dict["fulfillmentData"] = _items
        return _dict
        ...
        _obj = cls.model_validate(
            {
                "fulfillmentData": (
                    [
                        FulfillmentDataOrderDetails.from_dict(_item)
                        for _item in obj["fulfillmentData"]
                    ]
                    if obj.get("fulfillmentData") is not None
                    else None
                ),
