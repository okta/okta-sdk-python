...
class WsFederationApplication(Application):
    ...
    def name_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['template_wsfed']):
            raise ValueError("must be one of enum values ('template_wsfed')")
        return value
        ...
        _obj = cls.model_validate(
            {
                ...
                "name": obj.get("name"),
                "settings": WsFederationApplicationSettings.from_dict(obj["settings"]) if obj.get(
                    "settings"
                ) is not None else None
            }
        )
