...
#
# When to update this:
# 1. If integration tests fail with Pydantic validation errors about class identity
# 2. When new polymorphic model groups are added that reference field types
# 3. If new models are added as field types within existing discriminator groups
# 4. After OpenAPI spec updates that add new models to existing hierarchies
#
# How to identify missing dependencies:
# 1. Look for field type annotations in the parent and child models of discriminator groups
# 2. Check if those field types are model classes (not primitives like str, int, bool)
# 3. Add any model classes used as field types to the corresponding group's dependency list
#
# Testing:
# - Run `pytest tests/integration` to verify model deserialization works correctly
# - Pay special attention to tests that use `from_dict()` on polymorphic models
# - Test that `isinstance()` checks work with both `okta.ModelName` and `okta.models.ModelName`
#
