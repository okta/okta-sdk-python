...
class DeviceSignalCollectionPolicyRuleDeviceSignalCollection(BaseModel):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in device_context_providers (list)
        _items = []
        if self.device_context_providers:
            for _item in self.device_context_providers:
                if _item:
                    _items.append(_item.to_dict())
            _dict["deviceContextProviders"] = _items
        return _dict
        ...
        _obj = cls.model_validate(
            {
                "deviceContextProviders": (
                    [
                        DeviceContextProvider.from_dict(_item)
                        for _item in obj["deviceContextProviders"]
                    ]
                    if obj.get("deviceContextProviders") is not None
                    else None
                )
            }
        )
