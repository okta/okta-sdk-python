...
class DeviceContextProvider(BaseModel):
    ...
    def key_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['OKTA_VERIFY', 'CHROME_DEVICE_TRUST', 'DEVICE_POSTURE_IDP']):
            raise ValueError("must be one of enum values ('OKTA_VERIFY', 'CHROME_DEVICE_TRUST', 'DEVICE_POSTURE_IDP')")
        return value
