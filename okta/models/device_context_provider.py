...
class DeviceContextProvider(BaseModel):
    ...
    id: Optional[StrictStr] = Field(
        default=None, description="Unique identifier for the device context provider"
    )
    ...
    def key_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(
            ["OKTA_VERIFY", "CHROME_DEVICE_TRUST", "DEVICE_POSTURE_IDP"]
        ):
            raise ValueError(
                "must be one of enum values ('OKTA_VERIFY', 'CHROME_DEVICE_TRUST', 'DEVICE_POSTURE_IDP')"
            )
        return value

    @field_validator("user_identification")
    def user_identification_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(["ALLOW", "IGNORE"]):
            raise ValueError("must be one of enum values ('ALLOW', 'IGNORE')")
        return value
