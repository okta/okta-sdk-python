...
class SalesforceApplication(BaseModel):
    """
    Schema for the Salesforce app (key name: `salesforce`)  To create a Salesforce app, use the [Create an Application](
    /openapi/okta-management/management/tag/Application/#tag/Application/operation/createApplication) request with the
    following parameters in the request body. > **Note:** The Salesforce app only supports `BROWSER_PLUGIN`, `BOOKMARK`,
    and `SAML_2_0` sign-on modes.
    """  # noqa: E501
    ...
    def name_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['salesforce']):
            raise ValueError("must be one of enum values ('salesforce')")
        return value
    ...
    def sign_on_mode_validate_enum(cls, value):
        ...
        if value not in set(['BROWSER_PLUGIN', 'BOOKMARK', 'SAML_2_0']):
            raise ValueError("must be one of enum values ('BROWSER_PLUGIN', 'BOOKMARK', 'SAML_2_0')")
        return value
        ...
        _obj = cls.model_validate(
            {
                ...
                "settings": SalesforceApplicationSettings.from_dict(obj["settings"]) if obj.get(
                    "settings"
                ) is not None else None
            }
        )
