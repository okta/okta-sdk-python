...
class EmailPreviewLinks(BaseModel):
    """
    EmailPreviewLinks
    """ # noqa: E501
    var_self: Optional[HrefObjectSelfLink] = Field(default=None, alias="self")
    content_source: Optional[HrefObject] = Field(default=None, alias="contentSource")
    template: Optional[HrefObject] = None
    test: Optional[HrefObject] = None
    default_content: Optional[HrefObject] = Field(default=None, alias="defaultContent")
    __properties: ClassVar[List[str]] = ["self", "contentSource", "template", "test", "defaultContent"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of content_source
        if self.content_source:
            if not isinstance(self.content_source, dict):
                _dict['contentSource'] = self.content_source.to_dict()
            else:
                _dict['contentSource'] = self.content_source
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "contentSource": HrefObject.from_dict(obj["contentSource"]) if obj.get("contentSource") is not None else None,
            "template": HrefObject.from_dict(obj["template"]) if obj.get("template") is not None else None,
            "test": HrefObject.from_dict(obj["test"]) if obj.get("test") is not None else None,
            "defaultContent": HrefObject.from_dict(obj["defaultContent"]) if obj.get("defaultContent") is not None else None
        })
        return _obj
