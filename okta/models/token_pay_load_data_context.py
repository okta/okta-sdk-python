...
class TokenPayLoadDataContext(BaseModel):
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "request": (
                    InlineHookRequestObject.from_dict(obj["request"])
                    if obj.get("request") is not None
                    else None
                ),
                "session": (
                    BaseContextSession.from_dict(obj["session"])
                    if obj.get("session") is not None
                    else None
                ),
                "user": (
                    BaseContextUser.from_dict(obj["user"])
                    if obj.get("user") is not None
                    else None
                ),
                "protocol": (
                    TokenPayLoadDataContextAllOfProtocol.from_dict(obj["protocol"])
                    if obj.get("protocol") is not None
                    else None
                ),
                "policy": (
                    TokenPayLoadDataContextAllOfPolicy.from_dict(obj["policy"])
