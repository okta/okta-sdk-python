...
class TokenAuthorizationServerPolicyRuleAction(BaseModel):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of inline_hook
        if self.inline_hook:
            if not isinstance(self.inline_hook, dict):
                _dict["inlineHook"] = self.inline_hook.to_dict()
            else:
                _dict["inlineHook"] = self.inline_hook
        ...
        _obj = cls.model_validate(
            {
                ...
                "inlineHook": (
                    TokenAuthorizationServerPolicyRuleActionInlineHook.from_dict(
                        obj["inlineHook"]
                    )
                    if obj.get("inlineHook") is not None
                    else None
                ),
