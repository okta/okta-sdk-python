...
class BaseContextUserLinks(BaseModel):
    ...
    the user. These links are used to discover what groups the user is a part of and what factors they have enrolled.
    """  # noqa: E501

    groups: Optional[HrefObject] = Field(
        default=None,
        description="URL to retrieve the individual user's group memberships",
    )
    factors: Optional[HrefObject] = Field(
        default=None, description="URL to retrieve individual user's factor enrollments"
    )
        ...
        _obj = cls.model_validate(
            {
                ...
                "factors": (
                    HrefObject.from_dict(obj["factors"])
                    if obj.get("factors") is not None
                    else None
                ),
            }
        )
