...
class GracePeriod(BaseModel):
    ...
    def type_validate_enum(cls, value):
        ...
        if value not in set(["BY_DATE_TIME", "BY_DURATION"]):
            raise ValueError(
                "must be one of enum values ('BY_DATE_TIME', 'BY_DURATION')"
            )
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.expiry:
            if not isinstance(self.expiry, dict):
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "expiry": (
                    GracePeriodExpiry.from_dict(obj["expiry"])
                    if obj.get("expiry") is not None
                    else None
                ),
                "type": obj.get("type"),
            }
