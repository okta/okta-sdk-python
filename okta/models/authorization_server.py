...
class AuthorizationServer(BaseModel):
    ...
    audiences: Optional[List[StrictStr]] = Field(
        ...
        description="The recipients that the tokens are intended for. This becomes the `aud` claim in an access token. "
                    "Okta currently supports only one audience."
    )
    ...
    issuer_mode: Optional[StrictStr] = Field(
        ...
        description="Indicates which value is specified in the issuer of the tokens that a custom authorization server "
                    "returns: the Okta org domain URL or a custom domain URL.  `issuerMode` is visible if you have a "
                    "custom URL domain configured or the Dynamic Issuer Mode feature enabled. If you have a custom URL "
                    "domain configured, you can set a custom domain URL in a custom authorization server, "
                    "and this property is returned in the appropriate responses.  When set to `ORG_URL`, "
                    "then in responses, `issuer` is the Okta org domain URL: `https://${yourOktaDomain}`.  When set to "
                    "`CUSTOM_URL`, then in responses, `issuer` is the custom domain URL configured in the administration "
                    "user interface.  When set to `DYNAMIC`, then in responses, `issuer` is the custom domain URL if the "
                    "OAuth 2.0 request was sent to the custom domain, or is the Okta org's domain URL if the OAuth 2.0 "
                    "request was sent to the original Okta org domain.  After you configure a custom URL domain, "
                    "all new custom authorization servers use `CUSTOM_URL` by default. If the Dynamic Issuer Mode feature "
                    "is enabled, then all new custom authorization servers use `DYNAMIC` by default. All existing custom "
                    "authorization servers continue to use the original value until they're changed using the Admin "
                    "Console or the API. This way, existing integrations with the client and resource server continue to "
                    "work after the feature is enabled.",
        alias="issuerMode"
    )
    jwks: Optional[ResourceServerJsonWebKeys] = None
    jwks_uri: Optional[StrictStr] = Field(
        default=None,
        description="<x-lifecycle-container><x-lifecycle class=\"ea\"></x-lifecycle></x-lifecycle-container>URL string "
                    "that references a JSON Web Key Set for encrypting JWTs minted by the custom authorization server"
    )
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "accessTokenEncryptedResponseAlgorithm": obj.get("accessTokenEncryptedResponseAlgorithm"),
                "audiences": obj.get("audiences"),
                "created": obj.get("created"),
                "credentials": AuthorizationServerCredentials.from_dict(obj["credentials"]) if obj.get(
                    "credentials"
                ...
                "jwks": ResourceServerJsonWebKeys.from_dict(obj["jwks"]) if obj.get("jwks") is not None else None,
