...
class IDVEndpoints(BaseModel):
    """
    Contains endpoints for the IDV vendor. When you create an `IDV_STANDARD` IdP, you must include the `par`,
    `authorization`, `token`, and `jwks` endpoints in the request body.
    """  # noqa: E501
    ...
    __properties: ClassVar[List[str]] = ["authorization", "jwks", "par", "token"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of par
        if self.par:
            if not isinstance(self.par, dict):
                _dict["par"] = self.par.to_dict()
            else:
                _dict["par"] = self.par
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "authorization": (
                    IDVAuthorizationEndpoint.from_dict(obj["authorization"])
                ...
                "par": (
                    IDVParEndpoint.from_dict(obj["par"])
                    if obj.get("par") is not None
                    else None
                ),
                "token": (
                    IDVTokenEndpoint.from_dict(obj["token"])
