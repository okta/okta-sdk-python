...
class IdpPolicyRuleActionMatchCriteria(BaseModel):
    ...
    provider_expression: Optional[StrictStr] = Field(
        default=None,
        description="You can provide an Okta Expression Language expression with the Login Context that's evaluated with "
                    "the IdP. For example, the value `login.identifier` refers to the user's username. If the user is "
                    "signing in with the username `john.doe@mycompany.com`, the expression "
