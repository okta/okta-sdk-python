...
class OAuthMetadata(BaseModel):
    ...
    backchannel_authentication_request_signing_alg_values_supported: Optional[List[SigningAlgorithm]] = Field(
        default=None,
        description="<x-lifecycle-container><x-lifecycle class=\"oie\"></x-lifecycle></x-lifecycle-container>A list of "
                    "signing algorithms that this authorization server supports for signed requests."
    )
    ...
    subject_types_supported: Optional[List[SubjectType]] = Field(
        ...
        description="A list of the Subject Identifier types that this authorization server supports. Valid types include "
                    "`pairwise` and `public`, but only `public` is currently supported. See the [Subject Identifier "
                    "Types](https://openid.net/specs/openid-connect-core-1_0.html#SubjectIDTypes) section in the OpenID "
    ...
    def dpop_signing_alg_values_supported_validate_enum(cls, value):
        ...
        for i in value:
            if i not in set(['ES256', 'ES384', 'ES512', 'RS256', 'RS384', 'RS512']):
                raise ValueError("each list item must be one of ('ES256', 'ES384', 'ES512', 'RS256', 'RS384', 'RS512')")
        return value
