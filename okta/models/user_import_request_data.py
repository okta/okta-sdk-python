...
class UserImportRequestData(BaseModel):
    ...
    app_user: Optional[UserImportRequestDataAppUser] = Field(
        default=None, alias="appUser"
    )
    context: Optional[UserImportRequestDataContext] = None
    user: Optional[UserImportRequestDataUser] = None
    __properties: ClassVar[List[str]] = ["action", "appUser", "context", "user"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.app_user:
            if not isinstance(self.app_user, dict):
                _dict["appUser"] = self.app_user.to_dict()
            else:
                _dict["appUser"] = self.app_user
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "action": (
                    UserImportRequestDataAction.from_dict(obj["action"])
                    if obj.get("action") is not None
                    else None
                ),
                "appUser": (
                    UserImportRequestDataAppUser.from_dict(obj["appUser"])
                    if obj.get("appUser") is not None
                    else None
                ),
                "context": (
                    UserImportRequestDataContext.from_dict(obj["context"])
                    if obj.get("context") is not None
                    else None
                ),
                "user": (
                    UserImportRequestDataUser.from_dict(obj["user"])
