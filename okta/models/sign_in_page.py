...
class SignInPage(BaseModel):
    ...
    def widget_version_validate_regular_expression(cls, value):
        ...
        if not re.match(r"^(?:\*|\^?\d+(?:\.\d+){0,2})$", value):
            raise ValueError(r"must validate the regular expression /^(?:\*|\^?\d+(?:\.\d+){0,2})$/")
        return value
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.content_security_policy_setting:
            if not isinstance(self.content_security_policy_setting, dict):
                _dict['contentSecurityPolicySetting'] = self.content_security_policy_setting.to_dict()
            else:
                _dict['contentSecurityPolicySetting'] = self.content_security_policy_setting
