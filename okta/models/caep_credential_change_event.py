...
class CaepCredentialChangeEvent(BaseModel):
    ...
    fido2_aaguid: Optional[StrictStr] = Field(
        default=None, description="FIDO2 Authenticator Attestation GUID"
    )
    friendly_name: Optional[StrictStr] = Field(
        default=None, description="Credential friendly name"
    )
    ...
    def change_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(["create", "revoke", "update", "delete"]):
            raise ValueError(
                "must be one of enum values ('create', 'revoke', 'update', 'delete')"
            )
        return value
