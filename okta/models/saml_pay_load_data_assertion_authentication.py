...
class SAMLPayLoadDataAssertionAuthentication(BaseModel):
    ...
    authn_context: Optional[SAMLPayLoadDataAssertionAuthenticationAuthnContext] = Field(
        default=None, alias="authnContext"
    )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of authn_context
        if self.authn_context:
            if not isinstance(self.authn_context, dict):
                _dict["authnContext"] = self.authn_context.to_dict()
            else:
                _dict["authnContext"] = self.authn_context
        ...
        _obj = cls.model_validate(
            {
                ...
                "authnContext": (
                    SAMLPayLoadDataAssertionAuthenticationAuthnContext.from_dict(
                        obj["authnContext"]
                    )
                    if obj.get("authnContext") is not None
                    else None
                ),
            }
        )
