...
class BeforeScheduledActionPolicyRuleCondition(BaseModel):
    ...
    lifecycle_action: Optional[ScheduledUserLifecycleAction] = Field(
        default=None, alias="lifecycleAction"
    )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of lifecycle_action
        if self.lifecycle_action:
            if not isinstance(self.lifecycle_action, dict):
                _dict["lifecycleAction"] = self.lifecycle_action.to_dict()
            else:
                _dict["lifecycleAction"] = self.lifecycle_action
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "duration": (
                    Duration.from_dict(obj["duration"])
                    if obj.get("duration") is not None
                    else None
                ),
                "lifecycleAction": (
                    ScheduledUserLifecycleAction.from_dict(obj["lifecycleAction"])
                    if obj.get("lifecycleAction") is not None
                    else None
                ),
            }
        )
