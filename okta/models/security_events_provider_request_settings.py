...
class SecurityEventsProviderRequestSettings(BaseModel):
    ...
    # data type: SecurityEventsProviderSettingsSSFCompliant
    ...
    def actual_instance_must_validate_oneof(cls, v):
        error_messages = []
        match = 0
        # validate data type: SecurityEventsProviderSettingsSSFCompliant
        if not isinstance(v, SecurityEventsProviderSettingsSSFCompliant):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `SecurityEventsProviderSettingsSSFCompliant`"
            )
        else:
            match += 1
        # validate data type: SecurityEventsProviderSettingsNonSSFCompliant
        if not isinstance(v, SecurityEventsProviderSettingsNonSSFCompliant):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `SecurityEventsProviderSettingsNonSSFCompliant`"
            )
        else:
            match += 1
        if match > 1:
            ...
            raise ValueError(
                ...
                "schemas: SecurityEventsProviderSettingsNonSSFCompliant, SecurityEventsProviderSettingsSSFCompliant. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when setting `actual_instance` in SecurityEventsProviderRequestSettings with oneOf schemas: "
                "SecurityEventsProviderSettingsNonSSFCompliant, SecurityEventsProviderSettingsSSFCompliant. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into SecurityEventsProviderSettingsSSFCompliant
        try:
            instance.actual_instance = (
                SecurityEventsProviderSettingsSSFCompliant.from_json(json_str)
            )
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into SecurityEventsProviderSettingsNonSSFCompliant
        try:
            instance.actual_instance = (
                SecurityEventsProviderSettingsNonSSFCompliant.from_json(json_str)
            )
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
