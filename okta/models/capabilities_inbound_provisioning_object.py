...
class CapabilitiesInboundProvisioningObject(BaseModel):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of import_rules
        if self.import_rules:
            if not isinstance(self.import_rules, dict):
                _dict["importRules"] = self.import_rules.to_dict()
            else:
                _dict["importRules"] = self.import_rules

        # override the default output from pydantic by calling `to_dict()` of import_settings
        if self.import_settings:
            if not isinstance(self.import_settings, dict):
                _dict["importSettings"] = self.import_settings.to_dict()
            else:
                _dict["importSettings"] = self.import_settings
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "importRules": (
                    CapabilitiesImportRulesObject.from_dict(obj["importRules"])
                    if obj.get("importRules") is not None
                    else None
                ),
                "importSettings": (
                    CapabilitiesImportSettingsObject.from_dict(obj["importSettings"])
                    if obj.get("importSettings") is not None
                    else None
                ),
            }
        )
