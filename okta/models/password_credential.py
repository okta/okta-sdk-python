...
class PasswordCredential(BaseModel):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.hash:
            if not isinstance(self.hash, dict):
        ...
        if self.hook:
            if not isinstance(self.hook, dict):
                _dict["hook"] = self.hook.to_dict()
            else:
                _dict["hook"] = self.hook
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "hash": (
                    PasswordCredentialHash.from_dict(obj["hash"])
                    if obj.get("hash") is not None
                    else None
                ),
                "hook": (
                    PasswordCredentialHook.from_dict(obj["hook"])
                    if obj.get("hook") is not None
                    else None
                ),
                "value": obj.get("value"),
            }
