...
class OpenIdConnectApplicationSettingsClient(BaseModel):
    ...
    backchannel_token_delivery_mode: Optional[StrictStr] = Field(default=None, description="The delivery mode for Client-Initiated Backchannel Authentication (CIBA).  Currently, only `poll` is supported. > **Note:** This property appears for clients with `urn:openid:params:grant-type:ciba` defined as one of the `grant_types`. ")
    client_uri: Optional[Annotated[str, Field(strict=True, max_length=1024)]] = Field(default=None, description="URL string of a web page providing information about the client")
    consent_method: Optional[OpenIdConnectApplicationConsentMethod] = OpenIdConnectApplicationConsentMethod.TRUSTED
    dpop_bound_access_tokens: Optional[StrictBool] = Field(default=False, description="Indicates that the client application uses Demonstrating Proof-of-Possession (DPoP) for token requests. If `true`, the authorization server rejects token requests from this client that don't contain the DPoP header. > **Note:** If `dpop_bound_access_tokens` is true, then `client_credentials` and `implicit` aren't allowed in `grant_types`. ")
    frontchannel_logout_session_required: Optional[StrictBool] = Field(default=None, description="<x-lifecycle-container><x-lifecycle class=\"ea\"></x-lifecycle> <x-lifecycle class=\"oie\"></x-lifecycle></x-lifecycle-container>Determines whether Okta sends `sid` and `iss` in the logout request")
    frontchannel_logout_uri: Optional[Annotated[str, Field(strict=True, max_length=1024)]] = Field(default=None, description="<x-lifecycle-container><x-lifecycle class=\"ea\"></x-lifecycle> <x-lifecycle class=\"oie\"></x-lifecycle></x-lifecycle-container>URL where Okta sends the logout request")
    ...
    initiate_login_uri: Optional[StrictStr] = Field(default=None, description="URL string that a third party can use to initiate the sign-in flow by the client")
    ...
    jwks_uri: Optional[StrictStr] = Field(default=None, description="URL string that references a JSON Web Key Set for validating JWTs presented to Okta or for encrypting ID tokens minted by Okta for the client")
    logo_uri: Optional[Annotated[str, Field(strict=True, max_length=1024)]] = Field(default=None, description="The URL string that references a logo for the client. This logo appears on the client tile in the End-User Dashboard. It also appears on the client consent dialog during the client consent flow.")
    network: Optional[OpenIdConnectApplicationNetwork] = None
    participate_slo: Optional[StrictBool] = Field(default=None, description="<x-lifecycle-container><x-lifecycle class=\"ea\"></x-lifecycle> <x-lifecycle class=\"oie\"></x-lifecycle></x-lifecycle-container>Allows the app to participate in front-channel Single Logout  > **Note:** You can only enable `participate_slo` for `web` and `browser` application types (`application_type`). ")
    policy_uri: Optional[StrictStr] = Field(default=None, description="URL string of a web page providing the client's policy document")
    post_logout_redirect_uris: Optional[List[StrictStr]] = Field(default=None, description="Array of redirection URI strings for relying party-initiated logouts")
    ...
    request_object_signing_alg: Optional[StrictStr] = Field(default=None, description="The type of JSON Web Key Set (JWKS) algorithm that must be used for signing request objects")
    response_types: Optional[List[OAuthResponseType]] = Field(default=None, description="Array of OAuth 2.0 response type strings")
    sector_identifier_uri: Optional[Annotated[str, Field(strict=True, max_length=1024)]] = Field(default=None, description="The sector identifier used for pairwise `subject_type`. See [OIDC Pairwise Identifier Algorithm](https://openid.net/specs/openid-connect-messages-1_0-20.html#idtype.pairwise.alg)")
    subject_type: Optional[StrictStr] = Field(default=None, description="Type of the subject")
    tos_uri: Optional[StrictStr] = Field(default=None, description="URL string of a web page providing the client's terms of service document")
    wildcard_redirect: Optional[StrictStr] = Field(default=None, description="Indicates if the client is allowed to use wildcard matching of `redirect_uris`")
    __properties: ClassVar[List[str]] = ["application_type", "backchannel_authentication_request_signing_alg", "backchannel_custom_authenticator_id", "backchannel_token_delivery_mode", "client_uri", "consent_method", "dpop_bound_access_tokens", "frontchannel_logout_session_required", "frontchannel_logout_uri", "grant_types", "id_token_encrypted_response_alg", "idp_initiated_login", "initiate_login_uri", "issuer_mode", "jwks", "jwks_uri", "logo_uri", "network", "participate_slo", "policy_uri", "post_logout_redirect_uris", "redirect_uris", "refresh_token", "request_object_signing_alg", "response_types", "sector_identifier_uri", "subject_type", "tos_uri", "wildcard_redirect"]
    ...
    def backchannel_authentication_request_signing_alg_validate_enum(cls, value):
        ...
        if value not in set(['HS256', 'HS384', 'HS512', 'RS256', 'RS384', 'RS512', 'ES256', 'ES384', 'ES512']):
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "backchannel_authentication_request_signing_alg": obj.get("backchannel_authentication_request_signing_alg"),
            "backchannel_custom_authenticator_id": obj.get("backchannel_custom_authenticator_id"),
            "backchannel_token_delivery_mode": obj.get("backchannel_token_delivery_mode"),
            ...
            "id_token_encrypted_response_alg": obj.get("id_token_encrypted_response_alg"),
            ...
            "network": OpenIdConnectApplicationNetwork.from_dict(obj["network"]) if obj.get("network") is not None else None,
