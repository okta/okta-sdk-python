...
class PasswordCredentialHash(BaseModel):
    ...
    operations.  See the [Create user with imported hashed password](
    ...
    value: Optional[StrictStr] = Field(
        ...
        description="For SHA-512, SHA-256, SHA-1, MD5, and PBKDF2, this is the actual base64-encoded hash of the password "
                    "(and salt, if used). This is the Base64-encoded `value` of the SHA-512/SHA-256/SHA-1/MD5/PBKDF2 "
                    "digest that was computed by either pre-fixing or post-fixing the `salt` to the `password`, "
                    "depending on the `saltOrder`. If a `salt` was not used in the `source` system, then this should just "
                    "be the Base64-encoded `value` of the password's SHA-512/SHA-256/SHA-1/MD5/PBKDF2 digest. For BCRYPT, "
