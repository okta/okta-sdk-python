...
class SamlAssertionEncryption(BaseModel):
    ...
    x5c: Optional[List[StrictStr]] = Field(
        default=None,
        description="A list that contains exactly one x509 encoded certificate which Okta uses to encrypt the SAML assertion "
        "with",
    )
    ...
    def encryption_algorithm_validate_enum(cls, value):
        ...
        if value not in set(["AES128_CBC", "AES256_CBC", "AES256_CBC", "AES256_GCM"]):
            raise ValueError(
                "must be one of enum values ('AES128_CBC', 'AES256_CBC', 'AES256_CBC', 'AES256_GCM')"
            )
        return value

    @field_validator("key_transport_algorithm")
    def key_transport_algorithm_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(["RSA_OAEP", "RSA_15"]):
            raise ValueError("must be one of enum values ('RSA_OAEP', 'RSA_15')")
        return value
