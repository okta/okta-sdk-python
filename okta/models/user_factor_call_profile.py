...
class UserFactorCallProfile(BaseModel):
    ...
    phone_extension: Optional[Annotated[str, Field(strict=True, max_length=15)]] = (
        Field(
            default=None,
            description="Extension of the associated `phoneNumber`",
            alias="phoneExtension",
        )
    )
    phone_number: Optional[Annotated[str, Field(strict=True, max_length=15)]] = Field(
        default=None,
        description="Phone number of the factor. Format phone numbers to use the [E.164 standard]("
    ...
    def phone_number_validate_regular_expression(cls, value):
        ...
        if not re.match(r"^\+[1-9]\d{1,14}$", value):
            raise ValueError(
                r"must validate the regular expression /^\+[1-9]\d{1,14}$/"
            )
        return value
