...
class EnrollmentInitializationResponse(BaseModel):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in cred_requests (list)
        _items = []
        if self.cred_requests:
            for _item in self.cred_requests:
                if _item:
                    _items.append(_item.to_dict())
            _dict["credRequests"] = _items
        return _dict
        ...
        _obj = cls.model_validate(
            {
                "credRequests": (
                    [
                        WebAuthnCredRequest.from_dict(_item)
                        for _item in obj["credRequests"]
                    ]
                    if obj.get("credRequests") is not None
                    else None
                ),
