...
class CsrMetadata(BaseModel):
    ...
    subject_alt_names: Optional[CsrMetadataSubjectAltNames] = Field(
        default=None, alias="subjectAltNames"
    )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of subject_alt_names
        if self.subject_alt_names:
            if not isinstance(self.subject_alt_names, dict):
                _dict["subjectAltNames"] = self.subject_alt_names.to_dict()
            else:
                _dict["subjectAltNames"] = self.subject_alt_names
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "subject": (
                    CsrMetadataSubject.from_dict(obj["subject"])
                    if obj.get("subject") is not None
                    else None
                ),
                "subjectAltNames": (
                    CsrMetadataSubjectAltNames.from_dict(obj["subjectAltNames"])
                    if obj.get("subjectAltNames") is not None
                    else None
                ),
            }
        )
