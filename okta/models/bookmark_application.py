...
class BookmarkApplication(Application):
    ...
    def name_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['bookmark']):
            raise ValueError("must be one of enum values ('bookmark')")
        return value
        ...
        _obj = cls.model_validate(
            {
                ...
                "name": obj.get("name"),
                "settings": BookmarkApplicationSettings.from_dict(obj["settings"]) if obj.get(
                    "settings"
                ) is not None else None
            }
        )
