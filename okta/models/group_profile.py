...
class GroupProfile(BaseModel):
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = GroupProfile.model_construct()
        error_messages = []
        # validate data type: OktaUserGroupProfile
        if not isinstance(v, OktaUserGroupProfile):
            error_messages.append(f"Error! Input type `{type(v)}` is not `OktaUserGroupProfile`")
        else:
            return v

        # validate data type: OktaActiveDirectoryGroupProfile
        if not isinstance(v, OktaActiveDirectoryGroupProfile):
            error_messages.append(f"Error! Input type `{type(v)}` is not `OktaActiveDirectoryGroupProfile`")
        else:
            return v
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], OktaActiveDirectoryGroupProfile, OktaUserGroupProfile]]:
