...
class DNSRecordAuthenticators(BaseModel):
    """
    DNS TXT record that must be registered for an RP ID domain that requires verification. This is used to verify the domain
    ownership for the WebAuthn RP ID configuration. After the domain ownership is verified, the `DNSRecord` isn't returned
    in the response.
    """  # noqa: E501

    fqdn: Optional[StrictStr] = Field(default=None, description="The DNS record name")
    record_type: Optional[DNSRecordTypeAuthenticators] = Field(
