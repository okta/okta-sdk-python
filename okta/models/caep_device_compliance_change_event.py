...
class CaepDeviceComplianceChangeEvent(BaseModel):
    ...
    previous_status: StrictStr = Field(description="Previous device compliance status")
    ...
    def current_status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(["compliant", "not-compliant"]):
            raise ValueError(
                "must be one of enum values ('compliant', 'not-compliant')"
            )
        return value
