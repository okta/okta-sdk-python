...
class AuthenticatorKeyCustomAppAllOfSettings(BaseModel):
    ...
    app_instance_id: Optional[StrictStr] = Field(
        default=None,
        description="The application instance ID. For custom_app, you need to create an OIDC native app using the [Apps "
        "API](https://developer.okta.com/docs/reference/api/apps/) with `Authorization Code` and `Refresh Token` "
        "grant types. You can leave both `Sign-in redirect URIs` and `Sign-out redirect URIs` as the default "
        "values.",
        alias="appInstanceId",
    )
