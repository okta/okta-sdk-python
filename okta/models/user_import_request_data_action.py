...
class UserImportRequestDataAction(BaseModel):
    ...
    result: Optional[StrictStr] = Field(
        default=None,
        description="The current default action that results when Okta imports a user. The two possible values are "
        "`CREATE_USER` and `LINK_USER`. You can change the action that is taken by means of the commands object "
        "you return.",
    )
    ...
    def result_validate_enum(cls, value):
        ...
        if value not in set(["CREATE_USER", "LINK_USER"]):
            raise ValueError("must be one of enum values ('CREATE_USER', 'LINK_USER')")
        return value
