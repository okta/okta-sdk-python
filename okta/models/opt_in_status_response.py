...
class OptInStatusResponse(BaseModel):
    ...
    def opt_in_status_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(["OPTING_IN", "OPTED_IN", "OPTING_OUT", "OPTED_OUT"]):
            raise ValueError(
                "must be one of enum values ('OPTING_IN', 'OPTED_IN', 'OPTING_OUT', 'OPTED_OUT')"
            )
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "optInStatus": obj.get("optInStatus"),
                "_links": (
                    OptInStatusResponseLinks.from_dict(obj["_links"])
