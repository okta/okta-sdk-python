...
class Client:
    ...
    def __init__(self, input_config: dict = dict()):
        # Load configuration
        client_config_setter = ConfigSetter()
        client_config_setter.apply_config(input_config)
        ...
        self._api_token = self._config["client"]["token"]
        ...
        self._http_client.add_header({
            'User-Agent': ' '.join([self._config["client"]["userAgentExtra"],
                                    Client.DEFAULT_USER_AGENT])
            if self._config["client"]["userAgentExtra"]
    ...
    def _validate_config(self):
        ...
        if not errors:  # raise exception and stop program if errors
            newline = '\n'
