    ...
    def publish_csr_for_identity_provider_with_http_info(self, body, idp_id, csr_id, **kwargs):  # noqa: E501
        ...
        for key, val in six.iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method publish_csr_for_identity_provider" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'body' is set
        ...
        if ('idp_id' not in params or
                params['idp_id'] is None):
            raise ValueError("Missing the required parameter `idp_id` when calling `publish_csr_for_identity_provider`")  # noqa: E501
        # verify the required parameter 'csr_id' is set
        if ('csr_id' not in params or
                params['csr_id'] is None):
