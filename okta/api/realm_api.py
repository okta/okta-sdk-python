...
class RealmApi(ApiClient):
    ...
    async def create_realm(
        ...
        if Realm is Success:
            ...
            response, response_body, error = await self._request_executor.execute(request, Realm)
    ...
    async def list_realms(
        ...
        search: Annotated[Optional[StrictStr], Field(description="Searches for realms with a supported filtering expression for most properties.  Searches for realms can be filtered by the contains (`co`) operator. You can only use `co` with the `profile.name` property. See [Operators](https://developer.okta.com/docs/api/#operators).")] = None,
        sort_by: Annotated[Optional[StrictStr], Field(description="Specifies the field to sort by and can be any single property (for search queries only)")] = None,
