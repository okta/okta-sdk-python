...
class Authenticator(object):
    ...
    def update_authenticator_with_http_info(self, authenticator_id, body=None, **kwargs):  # noqa: E501
        ...
        all_params = ['authenticator_id', 'body']  # noqa: E501
        all_params.append('async_req')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')
        ...
        for key, val in six.iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method update_authenticator" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'authenticator_id' is set
        if ('authenticator_id' not in params or
                params['authenticator_id'] is None):
            raise ValueError("Missing the required parameter `authenticator_id` when calling `update_authenticator`")  # noqa: E501
