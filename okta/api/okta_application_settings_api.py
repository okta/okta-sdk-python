...
class OktaApplicationSettingsApi(ApiClient):
    ...
    async def get_first_party_app_settings(
        ...
        _response_types_map: Dict[str, Optional[str]] = {
            '200': "AdminConsoleSettings",
            '403': "Error",
            '429': "Error",
        }

        method, url, header_params, body, post_params = self._get_first_party_app_settings_serialize(
            app_name=app_name,
        ...
        if error:
            if AdminConsoleSettings is Success:
                return (None, error)
            else:
        ...
        if AdminConsoleSettings is Success:
            ...
            response, response_body, error = await self._request_executor.execute(request, AdminConsoleSettings)
    ...
    def _get_first_party_app_settings_serialize(
        ...
        return self.param_serialize(
            method='GET',
            resource_path='/api/v1/first-party-app-settings/{appName}',
    ...
    async def replace_first_party_app_settings(
        ...
        method, url, header_params, body, post_params = self._replace_first_party_app_settings_serialize(
            app_name=app_name,
            admin_console_settings=admin_console_settings,
            _request_auth=_request_auth,
            _content_type=_content_type,
            _headers=_headers,
            _host_index=_host_index
    ...
    def _replace_first_party_app_settings_serialize(
        ...
        # process the form parameters
        # process the body parameter
        if admin_console_settings is not None:
            _body_params = admin_console_settings
