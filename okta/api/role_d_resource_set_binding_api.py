...
class RoleDResourceSetBindingApi(ApiClient):
    ...
    async def create_resource_set_binding(
        ...
        Creates a binding for the resource set, custom role, and members (users or groups)  > **Note:** If you use a custom
        role with permissions that don't apply to the resources in the resource set, it doesn't affect the admin role. For
        example,  the `okta.users.userprofile.manage` permission gives the admin no privileges if it's granted to a resource
        set that only includes `https://{yourOktaDomain}/api/v1/groups/{targetGroupId}`  resources. If you want the admin to
        be able to manage the users within the group, the resource set must include the corresponding `https://{
        yourOktaDomain}/api/v1/groups/{targetGroupId}/users` resource.
    ...
    def _create_resource_set_binding_serialize(
        ...
        return self.param_serialize(
            method="POST",
            resource_path="/api/v1/iam/resource-sets/{resourceSetIdOrLabel}/bindings",
    ...
    def _delete_binding_serialize(
        ...
        return self.param_serialize(
            method="DELETE",
            resource_path="/api/v1/iam/resource-sets/{resourceSetIdOrLabel}/bindings/{roleIdOrLabel}",
            path_params=_path_params,
            query_params=_query_params,
            header_params=_header_params,
            body=_body_params,
