...
class CustomTemplatesApi(ApiClient):
    ...
    async def delete_all_customizations(
        ...
        Okta Email Templates is enabled, all customizations are deleted, including customizations for additional languages.
        If disabled, only customizations in Okta-supported languages are deleted.
    ...
    async def get_customization_preview(
        ...
        context. For example, `${user.profile.firstName}`.  <x-lifecycle class=\"ea\"></x-lifecycle> If Custom languages for
        Okta Email Templates is disabled, requests for the preview of an additional language customization by ID return a
    ...
    async def get_email_default_content(
        ...
        user's language given the following: - Custom languages for Okta Email Templates is enabled - An additional language
        is specified for the `language` parameter
    ...
    async def get_email_default_preview(
        ...
        Retrieves a preview of an Email Template's default content. All variable references are populated using the current
        user's context. For example, `${user.profile.firstName}`.  <x-lifecycle class=\"ea\"></x-lifecycle> Defaults to the
        current user's language given the following: - Custom languages for Okta Email Templates is enabled - An additional
    ...
    async def get_email_settings(
        ...
        if error:
            if EmailSettingsResponse is Success:
                return (None, error)
            else:
                return (None, None, error)
    ...
    async def get_email_template(
        ...
        if error:
            if EmailTemplateResponse is Success:
                return (None, error)
            else:
                return (None, None, error)
    ...
    async def list_email_customizations(
        ...
        Lists all customizations of an email template  <x-lifecycle class=\"ea\"></x-lifecycle> If Custom languages for Okta
        Email Templates is enabled, all existing customizations are retrieved, including customizations for additional
        languages. If disabled, only customizations for Okta-supported languages are returned.
    ...
    async def list_email_templates(
        ...
        _response_types_map: Dict[str, Optional[str]] = {
            "200": "List[EmailTemplateResponse]",
            "403": "Error",
            "404": "Error",
            "429": "Error",
        }
        ...
        if error:
            if List[EmailTemplateResponse] is Success:
                return (None, error)
            else:
                return (None, None, error)
    ...
    async def replace_email_customization(
        ...
        Replaces an email customization using property values  <x-lifecycle class=\"ea\"></x-lifecycle> If Custom languages
        for Okta Email Templates is disabled, requests to update a customization for an additional language return a `404
    ...
    async def send_test_email(
        ...
        <x-lifecycle class=\"ea\"></x-lifecycle> If Custom languages for Okta Email Templates is enabled and the `language`
        parameter is an additional language, the test email uses the customization corresponding to the language. 2. The
        email template's default customization 3. The email template's default content, translated to the current user's
        language  > **Note:** Super admins can view customized email templates with the **Send a test email** request.
        However, when custom email templates are sent to super admins as part of actual email notification flows,
        the customizations aren't applied. Instead, the default email template is used. This only applies to super admins.
